estLCCR = function(Y,H,model=c("loglin","logit"),W=NULL,X=NULL,N=NULL,biv=NULL,
                   flag=c("no","prev","sum","atleast"),
                   main=c("LC","same","Rasch"),
                   free_cov=c("no","class","resp","both"),
                   free_biv=c("no","class","int","both"),
                   free_flag=c("no","class","resp","both"),N0=NULL,beta0=NULL,lambda0=NULL,
                   control=list(),verb=TRUE,init_rand=FALSE,se_out=FALSE){

#---- check and fix input arguments ----
  if(H<1) stop("Number of classes cannot be smaller than 1")
...
#---- cycle ----
  ...
  it = 0
  lko = lk
  while((abs(lk-lko)/abs(lko)>control$reltol | it==0) & it<control$maxit){
    it = it+1
    lko = lk
...
#---- output ----
  AIC = -2*lk+2*(np)
  BIC = -2*lk+log(n)*(np)
  out = list(beta=beta,lambda=lambda,lk=lk,N=N,np=np,AIC=AIC,BIC=BIC,M=M,tauv=tauv,phiv=phiv,
             Piv=Piv,Q=Q,lk1=lk1,lk2=lk2,lk3=lk3,lk4=lk4,call = match.call(),
